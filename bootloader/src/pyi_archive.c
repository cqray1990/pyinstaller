...
/* Chunk size for file I/O and decompression */
...
_pyi_arch_extract_uncompressed(ARCHIVE_STATUS *status, TOC *ptoc, unsigned char *out)
{
     ...
     * of size_t type (files larger than 4 GB on 32-bit systems), although
     * in that case we would likely fail to allocate the data buffer in
     * the first place. But perform chunked read anyway, to keep the code
     * consistent with other codepaths, and to avoid compiler warnings about
     * possible overflow.
     */
    buffer = out;
    remaining_size = ptoc->ulen;
...
pyi_arch_open(ARCHIVE_STATUS *status)
{
    ...
    fseeko(status->fp, status->pkgstart + status->cookie.TOC, SEEK_SET);
